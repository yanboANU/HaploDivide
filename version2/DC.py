...
class DC(object):
    ...
    def __init__(self, k, data_start, data_end, all_clouds, clouds_at_index, contigs_at_index): # all_clouds, a cloud list
        ...
        self.enumerate_SNP_threshold = 20 
        print ("init", k, self.enumerate_length_threshold)
    ...
    def run_dc(self, clouds_at_index): # 12, Dec
        ...
        '''  
        print_aligned_region(11070, 11081, clouds_at_index)
        print_aligned_region(11195, 11206, clouds_at_index)
        sys.exit()        
        '''
        allHaps = []
        count = 1
        ...
        for block in self.blocks:  # blo/cks break at no reads cover region
            blockLength = block[-1] - block[0]
            print "phasing block: ", count, ", region ", block[0] , block[-1], "length", block[-1] - block[0]
            '''
            if blockLength > 500:
                self.enumerate_length_threshold = 5

            if blockLength > 2000:
                self.enumerate_length_threshold = 3
            '''    
            time1 = time.clock()
